/**
 * @file         gmarginalization_factor.cpp
...
#include "gmarginalization_factor.h"
	...
	bool MarginalizationFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
	{
		...
		for (int i = 0; i < static_cast<int>(marginalization_info->keep_block_size.size()); i++)
		{
			...
			else
			{
				...
				dx.segment<3>(idx + 3) = 2.0 * t_gfgo_utility::positify(Eigen::Quaterniond(x0(6), x0(3), x0(4), x0(5)).inverse() * Eigen::Quaterniond(x(6), x(3), x(4), x(5))).vec();
